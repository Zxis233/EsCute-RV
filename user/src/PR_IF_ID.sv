module PR_IF_ID (
    ...
    input  logic        fetch_access_fault_if_i,
    // IF级输出 给ID级输入
    output logic [31:0] pc_id_o,
    output logic [31:0] pc4_id_o,
    output logic [31:0] instr_id_o,
    output logic        fetch_access_fault_id_o,
    // 判断指令是否有效
    // 流水线冲刷时需要将指令置为无效
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            fetch_access_fault_id_o <= fetch_access_fault_id_o;
            instr_valid_id_o <= instr_valid_id_o;
        end else begin
            pc_id_o          <= pc_if_i;
            pc4_id_o         <= pc4_if_i;
            instr_id_o       <= instr_if_i;
            fetch_access_fault_id_o <= fetch_access_fault_if_i;
